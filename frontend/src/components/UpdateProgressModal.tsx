...
const UpdateProgressModal: React.FC<UpdateProgressModalProps> = ({ request, onClose, onSave }) => {
  ...
  const handleSave = () => {
    ...
    if (request.status === RequestStatus.Fulfilled && examDate) {
  ...
  return (
    <Modal isOpen={true} onClose={onClose}>
      <div className="p-6 bg-white dark:bg-slate-800 rounded-t-xl">
        <h3 className="text-lg font-bold leading-6 text-gray-900 dark:text-gray-100" id="modal-title">
      ...
      <div className="bg-gray-50 dark:bg-slate-700 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 rounded-b-xl">
        <button
          type="button"
          onClick={handleSave}
          disabled={ (request.status === RequestStatus.Fulfilled && !examDate) || (request.status === RequestStatus.ExamScheduled && !result) }
